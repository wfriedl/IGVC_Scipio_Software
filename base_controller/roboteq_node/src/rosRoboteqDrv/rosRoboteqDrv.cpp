...
void    RosRoboteqDrv::Initialize()
{
    try
    {
        ...
        _comunicator.IssueCommand("# 500"); // auto message response is 500ms
...
void    RosRoboteqDrv::CmdVelCallback(const geometry_msgs::Twist::ConstPtr& twist_velocity)
{
    ...
    {
        ROS_ERROR_STREAM_NAMED(NODE_NAME,"IssueCommand G1 Error: " << ex.what());
	throw;
    }
    catch(...)
    {
    ...
    if (rightVelRPM >= 0)
        ss << "!G 2 " << ((int) (rightVelRPM + 0.5) * 100);
    else
        ss << "!G 2 " << ((int) (rightVelRPM - 0.5) * 100);
    ...
    {
        ROS_ERROR_STREAM_NAMED(NODE_NAME,"IssueCommand G2 : " << ex.what());
	throw;
    }
    catch(...)
    {
...
geometry_msgs::Twist RosRoboteqDrv::ConvertWheelVelocityToTwist(float left_velocity, float right_velocity)
{
    ...
    float longitudinal_velocity = (left_velocity - right_velocity) * (WHEEL_DIAMETER_SCIPIO / 4);
        ...
        if( idx != string::npos )
        {
                ...
                if( idy != string::npos )
                {
                        ...
                        *pVal2 = 0L;
                        ...
                        ROS_INFO_STREAM("Wheel RPM's: " << wheelVelocity.left << " :: " << wheelVelocity.right);
                }
                else
		        {
